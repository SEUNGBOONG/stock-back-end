...
public class AccountDataPollingService {
    ...
    private final Map<String, AccountDataDTO> latestStockData = new ConcurrentHashMap<>();
    ...
    public void pollFluctuationData() throws Exception {
        ...
        List<AccountDataDTO> stockDataList = accountDataParser.parseAll(responseBody);

        if (stockDataList != null && !stockDataList.isEmpty()) {
            // 각 주식 이름을 키로 하여 최신 데이터를 저장
            for (AccountDataDTO stockData : stockDataList) {
                latestStockData.put(stockData.stockName(), stockData);
            }
        }
    }

    public AccountDataDTO getLatestStockData(String stockName) {
